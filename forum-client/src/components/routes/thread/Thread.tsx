...
const Thread = () => {
  ...
  const { id }: threadParams = useParams();
  ...
  const onClickPost = async (
    ...
    if (!userId || userId === "0") {
      ...
      if (!isNaN(createThreadMsg?.createThread?.messages[0])) {
    ...
    <div className="screen-root-container">
      ...
      <div className="thread-content-container">
        <div className="thread-content-post-container">
          ...
          <ThreadHeader
            userName={thread?.user?.Username ?? user?.Username}
            lastModifiedOn={
              thread?.lastModifiedOn
                ? new Date(`${thread?.lastModifiedOn}`)
                : new Date()
            }
        ...
        <div className="thread-content-points-container">
          <ThreadPointsBar
            points={thread?.points || 0}
            responseCount={thread?.threadItems?.length || 0}
